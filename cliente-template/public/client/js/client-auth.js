...
class ClientAuth {
  ...
  async handleLogin() {
    ...
    try {
      ...
      // ✅ Buscar cliente en Firestore GENERAL (sin autenticar)
  ...
  async onClientAuthenticated(clienteId, clienteData) {
    ...
    setTimeout(() => {
      if (window.clientDashboard) {
        ...
        console.warn("⚠️ clientDashboard no initialized yet");
      }
    }, 500);
  }
  ...
  setupStateChangeListener() {
    try {
      ...
      // Setup nuevo listener - NOTA: Este listener es opcional
      // El cliente ya está autenticado y no necesita validación de Firestore
      console.log(`✅ Listener de estado configurado (opcional)`);
      
    } catch (error) {
      console.error('⚠️ Error configurando listener de estado:', error.message);
