...
class ClientDashboard {
        ...
        <div class="row">
          ...
          <div class="col-md-6">
            <div class="card" style="border: none; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
              ...
              <div class="card-body">
                <button class="btn btn-sm btn-success mb-2 w-100" onclick="clientDashboard.importNewCity()">
                  <i class="bi bi-plus-circle"></i> Importar Nueva Ciudad
                </button>
  ...
  async loadStats() {
    try {
      ...
      // Intentar cargar estadísticas, pero no fallar si hay permisos insuficientes
      // (es común cuando hay multitenant con diferentes Firestore instances)
      
      try {
        // Contar cámaras públicas (intentar raíz primero)
        let camerasCount = 0;
        const countSnap = await firebase.firestore().collection('cameras').count().get();
        camerasCount = countSnap.data().count;
        
        // Contar cámaras privadas
        const countSnapPriv = await firebase.firestore().collection('cameras_privadas').count().get();
        const camerasPrivCount = countSnapPriv.data().count;
        
        this.dataStats.camaras = camerasCount + camerasPrivCount;
        
        // Contar siniestros
        const countSnapSin = await firebase.firestore().collection('siniestros').count().get();
        this.dataStats.siniestros = countSnapSin.data().count;
        
      } catch (firestoreError) {
        // Si falla por permisos, no mostrar error - es normal en multitenant
        console.debug('⚠️ No se pudieron cargar estadísticas de Firestore (permisos)', firestoreError.code);
        // Mantener valores por defecto (0 o últimos valores conocidos)
        this.dataStats.camaras = this.dataStats.camaras || 0;
        this.dataStats.siniestros = this.dataStats.siniestros || 0;
      }
      
      // Alertas (siempre 0 por ahora)
  ...
  async saveDataToFirestore(data, collectionName) {
    ...
    try {
      // ✅ IMPORTANTE: Usar Firestore del CLIENTE (laplatamaps-52a3b), NO el GENERAL
      const ref = this.clientDb.collection(`clientes/${clientId}/${collectionName}`);
      console.log(`💾 Referencia de Firestore creada (CLIENTE)`);
   ...
   */
  openProjectMap() {
    // Abrir mapa completo en la misma ventana
    window.location.href = 'https://trafico-map-general-v2.web.app/';
  }

  /**
   * Importar una nueva ciudad
   */
  importNewCity() {
    const cityName = prompt('¿Nombre de la nueva ciudad?');
    if (!cityName) return;
    
    alert(`✅ Solicitud enviada para importar: ${cityName}\n\nEn breve recibirás un correo con las instrucciones.`);
    console.log(`📍 Solicitud de nueva ciudad: ${cityName}`);
  }
}
