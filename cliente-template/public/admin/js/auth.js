...
class AdminAuth {
  ...
  async handleUserSignedIn(user) {
    ...
    // Obtener custom claims (rol) - FORZAR REFRESH
    console.log("Refrescando token para obtener custom claims frescos...");
    ...
    console.log("Role detectado:", this.role);

    // Validar que sea admin
    if (!this.isAdmin()) {
      console.warn("❌ User is not admin. Claims:", idTokenResult.claims);
      this.logout();
      this.showError("❌ No tienes permisos de administrador. Contacta al soporte.");
      return;
    }

    console.log("✅ Validación de admin exitosa");
    
    // Mostrar interfaz admin con pequeño delay para asegurar que todo está cargado
    setTimeout(() => {
      this.showAdminUI();
    }, 500);
