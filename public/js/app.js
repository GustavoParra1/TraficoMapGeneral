      ...
      setTimeout(() => {
        try {
          ...
          const handleQuestionFn = async (question) => {
            ...
            if (question.includes('Zonas calientes')) {
              try {
                ...
                const formatearDesgloseHorario = (desglose) => {
                  ...
                  if (conHora === 0) {
                    return `<p style="margin:4px 0 0 0; font-size:11px; color:#888;">Ninguno de estos ${total} evento(s) tiene hora registrada — no se puede calcular en qué franja horaria ocurren más.</p>`;
                  }
                  ...
                  const notaSinHora = sinHora > 0
                    ? `<p style="margin:6px 0 0 0; font-size:10px; color:#888;">(${sinHora} evento(s) sin hora registrada, no incluido(s) arriba.)</p>`
                    : '';
            ...
            // (watchPosition). "Conectado" = actualizó su ubicación en los
            // últimos UMBRAL_MINUTOS minutos. "Cerca mío" = dentro del
            // barrio oficial del cliente (mismo alcance que el resto de
            // las preguntas), no por distancia real.
            if (question.includes('Vecinos conectados cerca')) {
              try {
                if (typeof ZonaRiesgoLayer === 'undefined') {
                  FloatingWindow.show('⚠️ Error', '<p>ZonaRiesgoLayer no está cargado.</p>');
                  return;
                ...
                if (!dbVecinos || !clienteIdVecinos) {
                  FloatingWindow.show('⚠️ Error', '<p>No hay clienteId/base de datos disponible.</p>');
                  return;
                }

                const UMBRAL_MINUTOS = 5;
                const vcMarkersGroup = L.layerGroup();
                let vcUnsubscribe = null;

                const renderPanelVecinos = (vecinos) => {
                  const sinBarrio = !ZonaRiesgoLayer.tieneBarrioOficial();
                  const lista = vecinos.map((v) => `
                    <div style="display:flex; justify-content:space-between; font-size:12px; padding:4px 0; border-bottom:1px solid #f1f5f9;">
                      <span>👤 ${v.nombre}</span>
                      <span style="color:#888;">hace ${Math.max(0, Math.round((Date.now() - v.ts.getTime()) / 60000))} min</span>
                    </div>
                  `).join('') || '<p style="font-size:12px; color:#888;">Nadie conectado ahora mismo en esta zona.</p>';

                  return `
                    <p style="font-size:12px; color:#666; margin-bottom:8px;">Vecinos con actividad (GPS) en los últimos ${UMBRAL_MINUTOS} minutos.</p>
                    <p style="font-size:13px; font-weight:600; margin-bottom:8px;">Total conectados: ${vecinos.length}</p>
                    ${lista}
                    ${sinBarrio ? '<p style="margin-top:8px; font-size:10px; color:#b45309;">⚠️ Sin barrio oficial detectado — mostrando toda la zona disponible, sin recortar.</p>' : ''}
                  `;
                };

                const procesarSnapshotVecinos = (snapshot) => {
                  const ahora = Date.now();
                  const umbralMs = UMBRAL_MINUTOS * 60000;
                  const todos = snapshot.docs.map((doc) => {
                    ...
                    return {
                      ...
                      lng: d.lng,
                      ts,
                      nombre: d.nombre || d.email || 'Vecino'
                    };
                  }).filter((v) =>
                    typeof v.lat === 'number' && typeof v.lng === 'number' &&
                    v.ts && (ahora - v.ts.getTime()) <= umbralMs
                  );

                  const enBarrio = ZonaRiesgoLayer.filtrarPorBarrioOficialExterno(todos);

                  vcMarkersGroup.clearLayers();
                  enBarrio.forEach((v) => {
                    L.circleMarker([v.lat, v.lng], {
                      ...
                      fillColor: '#0ea5e9',
                      fillOpacity: 0.85
                    }).bindPopup(`👤 ${v.nombre}<br/>Activo hace ${Math.max(0, Math.round((Date.now() - v.ts.getTime()) / 60000))} min`)
                      .addTo(vcMarkersGroup);
                  });
                  vcMarkersGroup.addTo(map);

                  const contentEl = document.getElementById('vc-contenido');
                  if (contentEl) contentEl.innerHTML = renderPanelVecinos(enBarrio);
                };

                vcUnsubscribe = dbVecinos.collection(`clientes/${clienteIdVecinos}/vecinos`)
                  .onSnapshot(procesarSnapshotVecinos, (error) => {
                    console.error('❌ Error escuchando vecinos conectados:', error);
                  });

                FloatingWindow.show('👥 Vecinos conectados cerca mío', '<div id="vc-contenido">Cargando...</div>', {
                  width: '340px',
                  onClose: () => {
                    if (vcUnsubscribe) vcUnsubscribe();
                    if (map.hasLayer(vcMarkersGroup)) map.removeLayer(vcMarkersGroup);
                  }
                });
              } catch (error) {
                console.error('❌ Error en Vecinos conectados cerca mío:', error);
                FloatingWindow.show('⚠️ Error', `<p style="color: #dc2626;">${error.message}</p>`);
              }
              return;
            }
